...
class FastSelfAttentionEnc(nn.Module):
    ...
    def _restricted_attention(self, input_):
        ...
        attn_weight = attn_weight[
            :, :, (self.kernel_size - 1) // 2 : len_conv + ((self.kernel_size - 1) // 2)
        ]
    ...
    def forward(self, input_):
        ...
        sig = self.glu(input_)
        input_ = sig[:, :, : self.d_model] * F.sigmoid(sig[:, :, self.d_model :])

        # 2 - Apply LConv for the average info
        conv_loc = self._restricted_attention(input_)
...
class FastSelfAttentionDec(nn.Module):
    def __init__(self, n_head, d_model, d_conv, kernel_size, dropout=0.1):
        super().__init__()
        self.glu = nn.Linear(d_model, 2 * d_model)
        self.fc = nn.Linear(d_model, d_model, bias=False)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        ...
        self.multi_head_loc = FastSelfAttentionEnc(
            n_head, d_model, d_v, 15, dropout=dropout
        )
        self.multi_head_attn = MultiHeadAttention(
            n_head, d_model, d_k, d_v, dropout=dropout
        )
        self.ffn_fast = PositionwiseFeedForward(d_model, d_inner, dropout=dropout)
        self.ffn = PositionwiseFeedForward(d_model, d_inner, dropout=dropout)

    def forward(self, input, enc_qmask=None, enc_kmask=None):
        output = self.multi_head_loc(input)
        output = self.ffn_fast(output)
        output, enc_attn = self.multi_head_attn(
            output, output, output, qmask=enc_qmask, kmask=enc_kmask
        )
